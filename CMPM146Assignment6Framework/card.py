...
from action.agent_targeted_action import DealAttackDamage, ApplyStatus, AddBlock, Heal, DealDamage
...
class CardGen:
    ...
    SearingBlow = lambda: Card("SearingBlow", CardType.ATTACK, ConstValue(2), Character.IRON_CLAD, Rarity.UNCOMMON, DealAttackDamage(LinearUpgradable(12, 4)).To(ChooseAgentTarget(AgentSet.ENEMY)))
    ...
    BashStar = lambda: Card("BashStar", CardType.ATTACK, ConstValue(2), Character.IRON_CLAD, Rarity.STARTER, DealAttackDamage(UpgradableOnce(8, 10)).To(ChooseAgentTarget(AgentSet.ENEMY)).And(ApplyStatus(UpgradableOnce(2, 3), StatusEffectRepo.VULNERABLE).To(ChooseAgentTarget(AgentSet.ENEMY))))
    ...
    Flex = lambda: Card("Flex", CardType.SKILL, ConstValue(0), Character.IRON_CLAD, Rarity.COMMON, ApplyStatus(UpgradableOnce(2, 4), StatusEffectRepo.VIGOR).To(SelfAgentTarget()))
    SeeingRed = lambda: Card("SeeingRed", CardType.SKILL, UpgradableOnce(1, 0), Character.IRON_CLAD, Rarity.UNCOMMON, AddMana(ConstValue(2)), Exhaust().To(SelfCardTarget()))
    PommelStrike = lambda: Card("PommelStrike", CardType.ATTACK, ConstValue(1), Character.IRON_CLAD, Rarity.COMMON, DealAttackDamage(UpgradableOnce(9, 10)).To(ChooseAgentTarget(AgentSet.ENEMY)), DrawCard(UpgradableOnce(1, 2)))
    ShrugItOff = lambda: Card("ShrugItOff", CardType.SKILL, ConstValue(1), Character.IRON_CLAD, Rarity.COMMON, AddBlock(UpgradableOnce(8, 11)).To(SelfAgentTarget()), DrawCard())
    ...
    BloodLetting = lambda: Card("BloodLetting", CardType.SKILL, ConstValue(0), Character.IRON_CLAD, Rarity.UNCOMMON, DealDamage(ConstValue(3)).To(SelfAgentTarget()), AddMana(UpgradableOnce(2, 3)))
    ...
    Offering = lambda: Card("Offering", CardType.SKILL, ConstValue(0), Character.IRON_CLAD, Rarity.RARE, DealDamage(ConstValue(6)).To(SelfAgentTarget()), AddMana(ConstValue(2)), DrawCard(UpgradableOnce(3, 5)), Exhaust().To(SelfCardTarget()))
...
card_index = {}
for c in dir(CardGen):
    card_index[c] = getattr(CardGen, c)
...
class CardRepo:
    ...
    @staticmethod 
    def make_deck(cards):
        result = [card_index[c]() for c in cards]
        return result
